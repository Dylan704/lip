(** ⚠️ READ THIS ⚠️
    ...
    The test routine depends on a library called `ppx_expect`. To install
    `ppx_expect` run the command:

    ``` opam install ppx_expect ```

    And add `ppx_expect` next to `ppx_inline_test` in `test/dune`

    Here we assume that the `trace` function returns a `result` and never raises
    an exception. *)
...
(** ------------------------------------------ Types & definitions
    ...
    type trace_error = | TypeError of string | UnboundVar of ide | CannotMutate
    of ide | MovedValue of ide | MutBorrowOfNonMut of ide | DataRace of ide *
    mut * mut | OutOfGas of int | NotInLoop

    type 'output trace_result = ('output, trace_error) result * ['output] is a
    type parameter for successful executions *)
...
let string_of_trace_error = function
  ...
  | OtherErrors s -> spr "[OtherErrors] %s" s
...
let tests : (string * int * result) array =
  [|
    ("01-print.rs",           25, Ok "3\n4\n");
...
let%expect_test "test_trace" =
  Array.iter2
    (fun (name, prog) (_, gas, expected) ->
      let prog : Ast.cmd = Main.parse prog in
      ...
      *)
      let _, (actual : result) = Main.trace gas prog in
